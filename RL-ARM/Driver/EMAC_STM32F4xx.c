...
static int32_t Control (uint32_t control, uint32_t arg) {
  ...
  uint32_t dmaomr;
  uint32_t macffr;
	/*这个延时非常非常重要！！！，如果不加这个延时会造成非Debug模式下网口无法检测到插拔动作，我查了1天的BUG才找到这个问题！！！！*/
	osDelay(500);
	osDelay(500);
  if ((Emac.flags & EMAC_FLAG_POWER) == 0U) {
    return ARM_DRIVER_ERROR;
  ...
  switch (control) {
    case ARM_ETH_MAC_CONFIGURE:
      ...
      /* 配置 100MBit/10MBit 模式 */
      switch (arg & ARM_ETH_MAC_SPEED_Msk) {
//			switch (ARM_ETH_MAC_SPEED_10M & ARM_ETH_MAC_SPEED_Msk) {
        case ARM_ETH_MAC_SPEED_10M:
#if (ETH_MII == 0)
          /* RMII Half Duplex Colision detection does not workRMII 半双工冲突检测不工作 */
          maccr |= ETH_MACCR_DM;
#endif
          break;
